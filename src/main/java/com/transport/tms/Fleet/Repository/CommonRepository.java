...
public class CommonRepository {
    ...
    public List<DropdownData> getSiteList() {
        return queryDropdown("select FCY_0 as value, FCYNAM_0 as label from " + schemas.getX3Schema() + ".FACILITY");
    }

    public List<DropdownData> getCarrierList() {
        return queryDropdown("select BPTNUM_0, BPTNAM_0 from " + schemas.getX3Schema() + ".BPCARRIER");
    }

    public List<DropdownData> getBusinessLineList() {
        return queryDropdown(
            "select distinct IDENT2_0, TEXTE_0 from " + schemas.getX3Schema() + ".ATEXTRA " +
            "where IDENT1_0='425' and IDENT2_0 in (select CODE_0 from " + schemas.getX3Schema() + ".ATABDIV where NUMTAB_0=425) " +
            "and LANGUE_0 = 'ENG' and ZONE_0='LNGDES' order by IDENT2_0");
    }

    public List<DropdownData> getPrimaryLanguageList() {
        return queryDropdown(
            "select IDENT1_0, TEXTE_0 from " + schemas.getX3Schema() + ".ATEXTRA where IDENT1_0 in (select LAN_0 from " + schemas.getX3Schema() + ".TABLAN) " +
            "and ZONE_0='INTDES' and LANGUE_0='ENG' order by IDENT1_0");
    }

    public List<StyleData> getStyleList() {
        String sql = "select distinct DES_0, COD_0, STY_0 from " + schemas.getX3Schema() + ".ASTYLE order by COD_0";
    ...
    public List<DropdownData> getUnAvailableList() {
        return queryDropdown(
            "select IDENT1_0, TEXTE_0 from " + schemas.getX3Schema() + ".ATEXTRA " +
            "where IDENT1_0 in (select UVYCOD_0 from " + schemas.getX3Schema() + ".TABUNAVAIL) " +
    ...
    public List<DropdownData> getCountryList() {
        return queryDropdown(
            "select distinct IDENT1_0, TEXTE_0 from " + schemas.getX3Schema() + ".ATEXTRA where IDENT1_0 in (select CRY_0 from " + schemas.getX3Schema() + ".TABCOUNTRY) " +
    ...
    public List<PostalCodeDetails> getPostalDetailsList(String country) {
        String sql = "select CRY_0, POSCOD_0, POSCTY_0, SATCOD_0 from " + schemas.getX3Schema() + ".POSCOD where CRY_0=?";
    ...
    public List<DropdownData> getInspectionList() {
        return queryDropdown("select XID_0, XQDES_0 from " + schemas.getX3Schema() + ".XINSQUEH order by XID_0");
    }

    public List<DropdownData> getFixedAssetList() {
        return queryDropdown("select AASREF_0, AASDES1_0 from " + schemas.getX3Schema() + ".FXDASSETS order by AASREF_0");
    }
    ...
    public List<DropdownData> getCustomerList() {
        return queryDropdown("select BPCNUM_0, BPCNAM_0 from " + schemas.getX3Schema() + ".BPCUSTOMER order by BPCNUM_0");
    }

    public List<DropdownData> getCategoryList() {
        return queryDropdown(
            "select distinct TCLCOD_0, TEXTE_0 from " + schemas.getX3Schema() + ".ITMCATEG i " +
            "join " + schemas.getX3Schema() + ".ATEXTRA a on a.IDENT1_0 = i.TCLCOD_0 " +
            "where a.CODFIC_0='ITMCATEG' and a.LANGUE_0='ENG' and a.ZONE_0='TCLAXX' " +
            "order by TCLCOD_0");
    }

    public List<DropdownData> getVehicleClassList() {
        return queryDropdown("select CLASS_0, DES_0 from " + schemas.getX3Schema() + ".XX10CCLASS order by CLASS_0");
    }

    public List<DropdownData> getTrailerTypeList() {
        return queryDropdown("select XTRACOD_0, XDES_0 from " + schemas.getX3Schema() + ".XX10CXTRA order by XTRACOD_0");
    }

    public List<DropdownData> getDocumentTypeList() {
        return queryDropdown(
            "select IDENT2_0, TEXTE_0 from " + schemas.getX3Schema() + ".ATEXTRA where ZONE_0='LNGDES' " +
            "and IDENT1_0=1502 and IDENT2_0 in (select CODE_0 from " + schemas.getX3Schema() + ".ATABDIV where NUMTAB_0=1502)");
    }

    public List<DropdownData> getissueAuthList() {
        return queryDropdown(
            "select IDENT2_0, TEXTE_0 from " + schemas.getX3Schema() + ".ATEXTRA where ZONE_0='LNGDES' and IDENT1_0=1503 " +
            "and IDENT2_0 in (select CODE_0 from " + schemas.getX3Schema() + ".ATABDIV where NUMTAB_0=1503)");
    }

    public List<DropdownData> getLocalMenuList(Integer type) {
        String sql = "select LANNUM_0, LANMES_0 from " + schemas.getX3Schema() + ".APLSTD " +
                     "where LANCHP_0=" + type + " and LAN_0='ENG' and LANNUM_0<>0";
        return queryDropdown(sql);
    }

    public List<Map<String, Object>> getVehicleData() {
        String sql = "select CODEYVE_0, CATEGO_0, XCODOMETER_0, FCY_0 from " + schemas.getX3Schema() + ".XX10CVEHICUL";
