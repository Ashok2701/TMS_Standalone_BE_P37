...
public class X3CustomerRepository {
    ...
    public Integer count() {
        ...
        String sql = "SELECT COUNT(*) FROM " + x3 + ".BPCUSTOMER";
    ...
    public List<X3CustomerDTO> findCustomers() {
        String x3  = schemas.getX3Schema();
        String sql = "SELECT BPCNUM_0, BPCNAM_0, BPCSHO_0, BCGCOD_0 AS CRY_0, CUR_0"
                   + " FROM " + x3 + ".BPCUSTOMER";

        List<X3CustomerDTO> result = jdbcTemplate.query(sql, (rs, row) -> {
