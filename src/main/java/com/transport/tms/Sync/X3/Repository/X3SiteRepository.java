...
public class X3SiteRepository {
    ...
    public Integer count() {
        ...
        String sql = "SELECT COUNT(*) FROM " + x3 + ".FACILITY";
        Integer count = jdbcTemplate.queryForObject(sql, Integer.class);
        System.out.println("SQL SERVER SITE COUNT = " + count);
        return count;
    }

    public List<X3SiteDTO> findSites() {
        String x3  = schemas.getX3Schema();
        String sql = "SELECT F.FCY_0, F.FCYNAM_0, F.FCYSHO_0, F.CRY_0, F.BPAADD_0,"
                   + " A.BPADES_0, A.BPAADDLIG_0, A.BPAADDLIG_1, A.BPAADDLIG_2,"
                   + " A.POSCOD_0, A.CTY_0, A.SAT_0, A.CRYNAM_0"
                   + " FROM " + x3 + ".FACILITY F"
                   + " LEFT JOIN " + x3 + ".BPADDRESS A ON F.BPAADD_0 = A.BPAADD_0";

        return jdbcTemplate.query(sql, (rs, row) -> {
            X3SiteDTO dto = new X3SiteDTO();
            dto.setSiteCode(rs.getString("FCY_0"));
            dto.setSiteName(rs.getString("FCYNAM_0"));
            dto.setShortName(rs.getString("FCYSHO_0"));
